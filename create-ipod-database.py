...
parser.add_argument('-n', dest='naming', default=False, action='store_true', help='rename files to [a-zA-Z0-9]+.mp3')
...
for (dirpath, dirnames, filenames) in os.walk(opth.join('iPod_Control', 'Music')):
...
if (args.naming):
    def gen_base(f):
        _p, _f = opth.split(f)
        _b, _e = opth.splitext(_f)
        _r = "%04X"%rnd.randint(0, 0xFFFF)
        while _r in base_names:
            _r = "%04X"%rnd.randint(0, 0xFFFF)
        base_names.append(_r)
        nn = opth.join(_p, _r + _e)
        print("Old Name: ", f, " New name: ", nn)
        return nn
    print('Renaming all files to standard names.')
    for f in tracks:
        os.rename(f, gen_base(f))
    sys.exit(0)
...
with open('iPod_Control/iTunes/iTunesSD', 'wb') as f:
    f.write(db)
print('Done processing ', len(tracks), ' songs, enjoy!')
